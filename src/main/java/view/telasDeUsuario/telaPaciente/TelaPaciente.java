...
public class TelaPaciente extends JFrame {
    ...
    public TelaPaciente(Paciente paciente, TelaLogin telaLogin, Hospital hospital) {
        ...
        JSplitPane divisorDocs = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, painelEsquerdoDocs, painelDireitoVisualizacaoDocs);
        ...
        botaoVisualizarProntuario.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {gerarProntuario(txtPreviewProntuario, consultasAntList);}
        });
    ...
    private void gerarProntuario(JTextArea previewProntuario, JTable consultasAntList){
        ...
        Prontuario prontuario = paciente.getConsultasAnteriores().get(consultasAntList.getSelectedRow()).getProntuario();
        if (prontuario == null){
            JOptionPane.showMessageDialog(this, "Prontuário pendente", "Não foi possível gerar prontuario",  JOptionPane.WARNING_MESSAGE);
            return;
        }
